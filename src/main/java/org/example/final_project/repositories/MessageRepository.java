...
public interface MessageRepository extends JpaRepository<Message, Long>
{
    @Query("SELECT m FROM Message m WHERE "
            + "((m.sender.id = :currentUserId AND m.receiver.id = :peerId) OR "
            + "(m.sender.id = :peerId AND m.receiver.id = :currentUserId)) AND "
            + "(:firstPage = true OR m.sentAt < :beforeTime OR "
            + "(m.sentAt = :beforeTime AND m.id < :beforeId)) ORDER BY m.sentAt DESC, m.id DESC")
    List<Message> findConversationPage(@Param("currentUserId") Long currentUserId, @Param("peerId") Long peerId,
